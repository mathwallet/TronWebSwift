...
public struct TronABIEncoder {
    
}

extension TronABIEncoder {
    ...
    public static func encode(types: [TronABI.Element.InOut], values: [AnyObject]) -> Data? {
    ...
    public static func encode(types: [TronABI.Element.ParameterType], values: [AnyObject]) -> Data? {
        guard types.count == values.count else {return nil}
        var tails = [Data]()
        var heads = [Data]()
    ...
    public static func encodeSingleType(type: TronABI.Element.ParameterType, value: AnyObject) -> Data? {
        switch type {
        case .uint(_):
        ...
        case .address:
            if let string = value as? String, let address = TronAddress(string) {
                return Data(address.data.dropFirst(1)).setLengthLeft(32)
            } else if let address = value as? TronAddress {
                return Data(address.data.dropFirst(1)).setLengthLeft(32)
            } else if let data = value as? Data, let address = TronAddress(data) {
                return Data(address.data.dropFirst(1)).setLengthLeft(32)
            }
        case .bool:

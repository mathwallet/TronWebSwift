...
public struct TronABIDecoder {
    
}

extension TronABIDecoder {
    public static func decode(types: [TronABI.Element.InOut], data: Data) -> [AnyObject]? {
        let params = types.compactMap { (el) -> TronABI.Element.ParameterType in
            return el.type
        }
        return decode(types: params, data: data)
    }
    
    public static func decode(types: [TronABI.Element.ParameterType], data: Data) -> [AnyObject]? {
        //        print("Full data: \n" + data.toHexString())
    ...
    public static func decodeSignleType(type: TronABI.Element.ParameterType, data: Data, pointer: UInt64 = 0) -> (value: AnyObject?, bytesConsumed: UInt64?) {
    ...
    fileprivate static func followTheData(type: TronABI.Element.ParameterType, data: Data, pointer: UInt64 = 0) -> (elementEncoding: Data?, nextElementPointer: UInt64?) {
    ...
    public static func decodeLog(event: TronABI.Element.Event, eventLogTopics: [Data], eventLogData: Data) -> [String:Any]? {
        ...
        let nonIndexedTypes = nonIndexedInputs.compactMap { (inp) -> TronABI.Element.ParameterType in
            return inp.type
        }
        ...
        for i in 0 ..< indexedInputs.count {
            ...
            if !input.type.isStatic || input.type.isArray || input.type.memoryUsage != 32 {
                let (v, _) = TronABIDecoder.decodeSignleType(type: .bytes(length: 32), data: data)
                guard let valueUnwrapped = v else {return nil}
                indexedValues.append(valueUnwrapped)
            } else {
                let (v, _) = TronABIDecoder.decodeSignleType(type: input.type, data: data)
                guard let valueUnwrapped = v else {return nil}
                indexedValues.append(valueUnwrapped)
            }
        }
        let v = TronABIDecoder.decode(types: nonIndexedTypes, data: dataForProcessing)

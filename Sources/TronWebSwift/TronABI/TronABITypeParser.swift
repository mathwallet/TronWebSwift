...
public struct TronABITypeParser {
    ...
    static func baseTypeMatch(from string: String, length: UInt64 = 0) -> TronABI.Element.ParameterType? {
        ...
        case .string?:
            return .string
        case .tuple?:
            return .tuple(types: [TronABI.Element.ParameterType]())
        default:
            return nil
        }
    }
    
    public static func parseTypeString(_ string:String) throws -> TronABI.Element.ParameterType {
        let (type, tail) = recursiveParseType(string)
        guard let t = type, tail == nil else {throw TronABI.ParsingError.elementTypeInvalid}
        return t
    }
    
    static func recursiveParseType(_ string: String) -> (type: TronABI.Element.ParameterType?, tail: String?) {
        let matcher = try! NSRegularExpression(pattern: TronABI.TypeParsingExpressions.typeEatingRegex, options: NSRegularExpression.Options.dotMatchesLineSeparators)
        ...
        var tail: String = ""
        var type: TronABI.Element.ParameterType?
    ...
    static func recursiveParseArray(baseType: TronABI.Element.ParameterType, string: String) -> (type: TronABI.Element.ParameterType?, tail: String?) {
        let matcher = try! NSRegularExpression(pattern: TronABI.TypeParsingExpressions.arrayEatingRegex, options: NSRegularExpression.Options.dotMatchesLineSeparators)
        let match = matcher.matches(in: string, options: NSRegularExpression.MatchingOptions.anchored, range: string.fullNSRange)
        ...
        if match[0].numberOfRanges >= 2, let exactArrayRange = Range(match[0].range(at: 2), in: string) {
            ...
            let baseType = TronABI.Element.ParameterType.array(type: baseType, length: arrayLength)
            type = baseType
        } else {
            let baseType = TronABI.Element.ParameterType.array(type: baseType, length: 0)
            type = baseType
        }
